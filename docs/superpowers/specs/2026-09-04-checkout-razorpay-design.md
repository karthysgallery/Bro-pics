...
5. Compute `subtotal` (sum of `unitPrice * qty`), `shipping` (flat charge unless `subtotal >= freeShippingThreshold`, both read from `settings/global`), `discount: 0`, `total = subtotal - discount + shipping`.
